...
// Baseline is now pushed down (~-1.10) so the cylinder lives in the lower half
// of the viewport and the editorial poster has the upper half to itself.
let responsiveCylinderY = -1.10;
...
function onResize() {
  ...
  responsiveCylinderY = -1.10 - tallness * 0.5 - bigness * 0.45;
...
function applyState(dt) {
  ...
  cylinderGroup.scale.setScalar(0.75 * (1 - ease * 0.2));
  rings.forEach(r => {
...
for (let layer = 0; layer < TUNNEL_LAYERS; layer++) {
  for (let i = 0; i < TUNNEL_RING_COUNT; i++) {
    ...
    tunnelPos[k*3+2] = -1.10 - layer * 2.2;
    tunnelDepth[k] = layer;
